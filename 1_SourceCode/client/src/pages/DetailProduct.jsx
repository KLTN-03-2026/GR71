...
import { requestToggleFavourite } from '../config/FavouriteRequest';
...
function DetailProduct() {
    ...
    const isFavourited = favourite.some((f) => f.userId?._id === dataUser?._id || f.userId === dataUser?._id);

    const handleCreateFavourite = async () => {
        if (!dataUser?._id) {
            toast.warning('Vui lòng đăng nhập để lưu tour yêu thích!');
            navigate('/login');
            return;
        }
        try {
            const res = await requestToggleFavourite(product._id);
            fetchProduct();
            if (res.metadata?.action === 'added') {
                                    ...
                                    <Card className="overflow-hidden shadow-2xl border-0 bg-white/80 backdrop-blur-sm rounded-3xl">
                                        <div className="relative">
                                            ...
                                            <div className="absolute bottom-6 right-6 flex gap-2">
                                                <div className="bg-white/90 backdrop-blur-sm p-3 rounded-full shadow-lg hover:bg-white transform hover:scale-110 transition-all duration-300 cursor-pointer">
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <Space direction="vertical" className="w-full">
                                                    ...
                                                    <Button
                                                        ...
                                                        block
                                                        className={`h-12 border font-semibold rounded-full backdrop-blur-sm transition-all duration-300 flex items-center justify-center gap-2 ${
                                                            isFavourited
                                                                ? 'bg-white text-red-500 border-white hover:bg-red-50'
