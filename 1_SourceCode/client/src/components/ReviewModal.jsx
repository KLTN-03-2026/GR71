...
// reviewedProductIds: Set of productId strings đã được review trong đơn này
function ReviewModal({ visible, onClose, order, reviewedProductIds = new Set() }) {
    const [rating, setRating] = useState(0);
    const [content, setContent] = useState('');
    const [loading, setLoading] = useState(false);
    const [selectedTourIndex, setSelectedTourIndex] = useState(0);

    // Các tour chưa được review trong đơn này
    const pendingItems = order?.items?.filter(
        (item) => !reviewedProductIds.has(item.product?._id?.toString())
    ) ?? [];
    ...
    const handleSubmit = async () => {
        if (pendingItems.length === 0) {
            message.info('Tất cả tour trong đơn này đã được đánh giá!');
            return;
        }
        ...
        const selectedTour = pendingItems[selectedTourIndex];
        if (!selectedTour) {
        ...
        >
            {pendingItems.length === 0 ? (
                <div className="text-center py-8">
                    ...
                    <Title level={5} className="text-gray-700">Bạn đã đánh giá tất cả tour trong đơn hàng này!</Title>
                    <Button onClick={handleCancel} className="mt-4">Đóng</Button>
                </div>
            ) : (
                <div className="space-y-6">
                                            ...
                                            >
                                                <div className="flex gap-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <Title level={5} className="!mb-1 !font-bold text-gray-800 line-clamp-1 text-sm">
                                                            {item.product?.title}
                                                        </Title>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <Tag color="blue" size="small">{item.product?.destination}</Tag>
                                                        </div>
                    ...
                    {/* Selected Tour Information (single tour) */}
                    {pendingItems.length === 1 && (
                        <Card className="bg-gray-50 border-0 shadow-sm">
                            <div className="flex gap-4">
                                <div className="flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden">
                                    <Image
                                        src={
                                            pendingItems[0].product?.images?.[0]
                                                ? `${import.meta.env.VITE_API_URL}/uploads/products/${pendingItems[0].product.images[0]}`
                                                : 'https://via.placeholder.com/200'
                                        }
                                        alt={pendingItems[0].product?.title}
                                        className="w-full h-full object-cover"
                                        preview={false}
                                    />
                                </div>
                                <div className="flex-1">
                                    <Title level={5} className="!mb-2 !font-bold text-gray-800 line-clamp-2">
                                        {pendingItems[0].product?.title}
                                    </Title>
                                    <div className="flex items-center gap-2">
                                        <Tag color="blue">{pendingItems[0].product?.destination}</Tag>
                                    </div>
                                </div>
                            </div>
                        </Card>
