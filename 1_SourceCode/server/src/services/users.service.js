...
class UserService {
    async createUser(data) {
        ...
        if (!passwordRegex.test(password)) {
            throw new BadRequestError('Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường và chữ số!');
        }
    ...
    async updateUser(id, data) {
        ...
        if (email && email !== user.email) {
            const emailTaken = await modelUser.findOne({ email, _id: { $ne: id } });
            if (emailTaken) {
    ...
    async forgotPassword(email) {
        ...
        // Chỉ trả token, KHÔNG trả OTP — OTP chỉ gửi qua email
        return { token };
    }
    ...
    async resetPassword(token, otpUser, newPassword) {
        ...
        // Xóa toàn bộ OTP của user này ngay sau khi verify thành công — tránh dùng lại
